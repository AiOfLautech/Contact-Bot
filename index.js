...
const { default: makeWASocket, Browsers, useMultiFileAuthState } = require('@adiwajshing/baileys');
...
async function initWhatsAppConnection(ctx) {
  ...
  try {
    ...
    console.error('Failed to initialize auth state:', err.message);
  ...
  try {
    ...
    console.error('Failed to create WhatsApp socket:', err.message);
  ...
  client.ev.on('connection.update', async (update) => {
    ...
    if (update.connection === 'open') {
      ...
      try {
        ...
        console.error('Broadcast error in connection update:', err.message);
        ctx.reply('❌ Broadcast failed during connection update.');
      }
    } 
  ...
  try {
    ...
    for (const [index, contact] of contacts.entries()) {
      try {
        ...
        console.error(`Failed to send to ${contact.tel}:`, error.message);
        failed++;
