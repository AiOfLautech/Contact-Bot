...
// Contact selection handler - FIXED "message not modified" error
bot.action(/toggle_(\d+)/, async (ctx) => {
  ...
  if (!ctx.session.selectedContacts.includes(contact)) {
    ...
    selectionMessage = `✅ Selected ${contact.name}`;
    changed = true;
  } else {
    ctx.session.selectedContacts = ctx.session.selectedContacts.filter(c => c.tel !== contact.tel);
    selectionMessage = `❌ Removed ${contact.name}`;
    changed = true;
  }
  
  ctx.answerCbQuery(selectionMessage);
...
// Select all/Clear all handlers - FIXED "message not modified" error
bot.action(['select_all', 'clear_all'], async (ctx) => {
  ...
  if (ctx.match[0] === 'select_all') {
    ...
    if (ctx.session.selectedContacts?.length !== ctx.session.contacts.length) {
      ...
      actionMessage = '✅ All contacts selected';
      changed = true;
    } else {
      actionMessage = 'All contacts already selected';
    }
  } else {
    // Only clear if there are selected contacts
    if (ctx.session.selectedContacts?.length > 0) {
      ctx.session.selectedContacts = [];
      actionMessage = '❌ All contacts cleared';
      changed = true;
    } else {
      actionMessage = 'No contacts to clear';
    }
  }
  
  ctx.answerCbQuery(actionMessage);
...
// Broadcast messages - FIXED with better progress and error handling
async function broadcastMessages(ctx) {
  ...
  try {
    ...
    try {
      progressMessage = await ctx.reply('📤 Broadcast progress: 0%');
    } catch (e) {
    ...
    for (const [index, contact] of contacts.entries()) {
      try {
        ...
        if (progressMessage && ((index + 1) % 5 === 0 || index === contacts.length - 1)) {
          ...
          try {
            ...
            console.error('Failed to update progress:', editError);
          }
        }
        
        // Send individual success notification for every contact
        try {
          await ctx.reply(`📬 Message sent to: ${contact.name} (${contact.tel})`);
        } catch (notifyError) {
        ...
        try {
          await ctx.reply(`❌ Failed to send to: ${contact.name} (${contact.tel})\nError: ${error.message}`);
        } catch (notifyError) {
