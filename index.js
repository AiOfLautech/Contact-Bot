...
// Contact selection handler with error fix
...
// Select all/Clear all handlers with error fix
...
// Initialize WhatsApp connection with FIXED authentication
async function initWhatsAppConnection(ctx) {
  ...
  // Initialize WhatsApp client with FIXED auth
  const sessionFile = path.join(sessionDir, `${userId}.json`);
  
  // FIXED: Use the correct authentication method
...
// Broadcast messages with enhanced error handling
