...
function useSingleFileAuthState(file) {
  let creds = {};
  let keys = {};
  ...
  const saveState = () => {
    try {
      const data = JSON.stringify({ creds, keys }, null, 2);
  ...
  return {
    state: {
      ...
      keys: {
        get: (type, ids) => {
          return ids.map(id => keys[id]?.[type]);
        },
        set: (data) => {
...
async function initWhatsAppConnection(ctx) {
  ...
  // Save state when credentials update
...
async function broadcastMessages(ctx) {
  ...
  try {
    ...
    const progressMessage = await ctx.reply('📤 Broadcast progress: 0%');
    ...
    for (const [index, contact] of contacts.entries()) {
      try {
        ...
        if ((index + 1) % 5 === 0 || index === contacts.length - 1) {
